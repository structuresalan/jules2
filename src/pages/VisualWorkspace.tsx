...
type MarkupType = 'arrow' | 'cloud' | 'text' | 'box' | 'callout' | 'pen' | 'highlighter' | 'dimension' | 'distance';
...
function TB({
  tid, active, onClick, icon, label, toggle,
}: {
...
function Sep() {
  return <div className="w-px h-9 bg-slate-700 mx-0.5 self-center shrink-0" />;
}
...
export function VisualWorkspace() {
  ...
  // Drawing state (refs for event handlers, state for rendering preview)
  ...
  const undo = useCallback(() => {
    ...
    hist.current.idx--;
    const snap = hist.current.snaps[hist.current.idx];
    setBoardMarkups(snap);
    setHistIdx(hist.current.idx);
    setSelectedId(null);
  }, []);
  ...
  const toPt = useCallback((e: PointerEvent | React.PointerEvent): Pt => {
  ...
  const onPointerDown = useCallback((e: React.PointerEvent) => {
    ...
    if (t === 'eraser') {
      const hit = hitTest(pt, markups);
      if (hit) {
        updateMarkups(activeBoardId, prev => prev.filter(m => m.id !== hit.id));
        setSelectedId(null);
      }
      return;
    }
  ...
  }, [tool, markups, activeBoardId, toPt, hitTest, updateMarkups, zoomAt, fitView, scaleSet, setTool]);
  ...
  const onPointerUp = useCallback((_e: React.PointerEvent) => {
    ...
    if (t === 'pen' || t === 'highlighter') {
      ...
      const m: Markup = {
        id: genId(), type: t, number: counter,
        points: pts, text: '', color: col,
        strokeWidth: t === 'highlighter' ? 14 : 2, fontSize: 14,
    ...
    if (t === 'zoom-area') {
      const rect = containerRef.current!.getBoundingClientRect();
      const centerX = ((start.x + cur.x) / 2) * zoomRef.current + panRef.current.x;
      const centerY = ((start.y + cur.y) / 2) * zoomRef.current + panRef.current.y;
      const factor = Math.min(rect.width / (dx * zoomRef.current + 1), rect.height / (dy * zoomRef.current + 1));
      const nz = Math.min(zoomRef.current * factor * 0.85, 10);
      setPan({ x: rect.width / 2 - centerX * (nz / zoomRef.current), y: rect.height / 2 - centerY * (nz / zoomRef.current) });
      setZoom(nz);
      return;
    }
  ...
  }, [selectedId, activeBoardId, updateMarkups, undo, redo, setTool]);
  ...
  const renderMarkup = (m: Markup) => {
    ...
    const hitRect = (
      <rect
        key={`hit-${m.id}`}
        data-testid={`annotation-hit-${m.number}`}
        x={bx - hitPad} y={by - hitPad}
        width={bw + hitPad * 2} height={bh + hitPad * 2}
    ...
    const testProps = { 'data-testid': `annotation-${m.number}`, 'data-tool-type': m.type };

    if (m.type === 'box') {
      shape = <rect {...testProps} key={m.id} x={bx} y={by} width={bw} height={bh} fill="none" stroke={stroke} strokeWidth={sw} style={{ filter: filt }} />;
    } else if (m.type === 'arrow') {
      const mid = `ah-${m.id}`;
      const ms = 8 / zoom;
      shape = (
        <g key={m.id} {...testProps} style={{ filter: filt }}>
          <defs>
      ...
      shape = <path {...testProps} key={m.id} d={cloudPath(bx, by, bw, bh)} fill="none" stroke={stroke} strokeWidth={sw} style={{ filter: filt }} />;
    } else if (m.type === 'text') {
      shape = (
        <text {...testProps} key={m.id} x={m.points[0].x} y={m.points[0].y}
      ...
      const cx = (bx + bx + bw) / 2, cy = by + bh / 2;
      shape = (
        <g key={m.id} {...testProps} style={{ filter: filt }}>
          ...
          <text x={bx + 5 / zoom} y={by + bh / 2} fill="#1e293b" fontSize={m.fontSize / zoom}
            fontFamily="sans-serif" dominantBaseline="middle">{m.text}</text>
        </g>
      );
    } else if (m.type === 'pen' || m.type === 'highlighter') {
      const pts = m.points.map(p => `${p.x},${p.y}`).join(' ');
      shape = <polyline {...testProps} key={m.id} points={pts} fill="none" stroke={stroke} strokeWidth={sw}
  ...
  const renderPreview = () => {
    ...
    if (type === 'box' || type === 'zoom-area' || type === 'area' || type === 'angle')
      return <rect x={x} y={y} width={w} height={h} fill={type === 'zoom-area' ? 'rgba(59,130,246,0.1)' : 'none'} stroke={color} strokeWidth={sw} strokeDasharray={dash} />;
    if (type === 'cloud')
      return <path d={cloudPath(x, y, w, h)} fill="none" stroke={color} strokeWidth={sw} strokeDasharray={dash} />;
    if (type === 'arrow') {
      const mid = 'prev-arrowhead';
      const ms = 10 / zoom;
      return (
        <g>
          <defs>
          ...
          <line x1={start.x} y1={start.y} x2={cur.x} y2={cur.y} stroke={color} strokeWidth={sw} markerEnd={`url(#${mid})`} />
        </g>
      );
    }
    if (type === 'text')
    ...
    if (type === 'callout')
      return (
        <g>
          <line x1={start.x} y1={start.y} x2={(x + x + w) / 2} y2={(y + y + h) / 2} stroke={color} strokeWidth={sw} strokeDasharray={dash} />
  ...
  const renderGrid = () => {
    ...
    const x0 = Math.floor(-pan.x / zoom / g) * g;
    const y0 = Math.floor(-pan.y / zoom / g) * g;
  ...
  const renderHandles = () => {
    ...
    const b = bounds(selectedMarkup);
    const pad = 8 / zoom, hw = 4 / zoom;
    const corners = [
      { x: b.x - pad, y: b.y - pad }, { x: b.x + b.w + pad, y: b.y - pad },
      { x: b.x - pad, y: b.y + b.h + pad }, { x: b.x + b.w + pad, y: b.y + b.h + pad },
    ];
    return (
      <g>
        ...
        {corners.map((c, i) => <rect key={i} x={c.x - hw} y={c.y - hw} width={hw * 2} height={hw * 2} fill="#3b82f6" />)}
      </g>
    );
  };
  ...
  const cursor = isPanning.current ? 'grabbing' : {
    select: 'default', pan: 'grab', 'zoom-in': 'zoom-in', zoom: 'zoom-in',
    'zoom-out': 'zoom-out', 'zoom-area': 'crosshair', eraser: 'cell',
    text: 'crosshair',
  }[tool as string] ?? 'crosshair';
  ...
  // ── Tool handler helper (sets tool + closes panels) ───────────────────────
  const activateTool = useCallback((t: Tool) => {
    setTool(t);
    if (!['color','photo','file','note','link','scale'].includes(t)) {
      setActivePanel(null);
    }
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900 text-slate-200 overflow-hidden select-none">
      ...
      {/* ── Header tabs ──────────────────────────────────────────────────── */}
      <div className="flex items-center justify-between px-4 py-0 bg-slate-950 border-b border-slate-800 shrink-0 h-9">
        ...
        <span className="text-xs text-slate-500 pr-1">Project: 1234 – Riverside Office Building</span>
      </div>
      ...
      <div className="flex items-center gap-0 px-1 py-1 bg-slate-800 border-b border-slate-700 overflow-x-auto shrink-0">
        ...
        <Sep/>
        <TB tid="tool-color"  active={activePanel==='color'} onClick={()=>activateTool('color')}  icon={<div className="w-4 h-4 rounded-full border-2 border-slate-400 shrink-0" style={{background:color}}/>} label="Color"/>
        <Sep/>
        ...
        <span
          ...
          data-active-tool={TOOL_NAMES[tool] ?? tool}
      ...
      <div className="flex flex-1 overflow-hidden">
                  ...
                  <div key={cat.id}>
                    ...
                    {isOpen && children.map(child => (
                      <button
                        ...
                        onClick={() => setActiveBoardId(child.id)}
                        className={`flex items-center w-full pl-7 pr-3 py-1 truncate transition-colors ${
          ...
          >
            <svg width={canvasSize.w} height={canvasSize.h} className="absolute inset-0">
              <g
                ...
                data-plan-pan-x={String(pan.x)}
                data-plan-pan-y={String(pan.y)}
                data-plan-zoom={String(zoom)}
            ...
            {!bgImage && (
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="text-center text-slate-500">
                  ...
                  <p className="text-sm">Drop an image or click Upload to set background</p>
                  <p className="text-xs mt-1 opacity-50">JPG · PNG · PDF (image)</p>
                </div>
              </div>
            )}
          ...
          <div className="h-44 shrink-0 border-t border-slate-700 bg-slate-800 overflow-auto">
            <div className="flex items-center justify-between px-4 py-1.5 border-b border-slate-700 sticky top-0 bg-slate-800 z-10">
              ...
              <div className="flex gap-2">
                <button aria-label="Reset active board" onClick={() => { updateMarkups(activeBoardId, () => activeBoardId === 'b1' ? SEED : []); setSelectedId(null); }}
            ...
            {markups.length === 0
              ? <div className="px-4 py-6 text-xs text-slate-500 text-center">No markups. Draw on the canvas to start annotating.</div>
              : (
                    ...
                    {markups.map(m => (
                      <tr key={m.id} onClick={() => setSelectedId(m.id)}
                        ...
                        <td className="px-4 py-1">
                          <span className="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold text-white shrink-0" style={{background:m.color}}>{m.number}</span>
                        </td>
          ...
          <div className="w-60 shrink-0 flex flex-col bg-slate-800 border-l border-slate-700 overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto">
              <div className="flex items-center justify-between px-3 py-1.5 border-b border-slate-700 sticky top-0 bg-slate-800 z-10">
                ...
                {selectedMarkup && (
                  <button onClick={() => setSelectedId(null)} className="text-slate-500 hover:text-white"><X size={11}/></button>
                )}
              </div>

              {!selectedMarkup ? (
                <div className="px-4 py-6 text-xs text-slate-500 text-center">Select a markup to inspect properties.</div>
              ) : (
                    ...
                    <div className="flex gap-1.5 flex-wrap">
                      {COLORS.map(c => (
                        <button key={c} onClick={() => setBoardMarkups(prev => ({ ...prev, [activeBoardId]: (prev[activeBoardId]??[]).map(m => m.id===selectedId ? {...m,color:c} : m) }))}
                          className={`w-5 h-5 rounded-full border-2 ${selectedMarkup.color===c ? 'border-white' : 'border-transparent'}`}
                          style={{background:c}}/>
                      ))}
                    </div>
                  </div>
        ...
        {!showInspector && (
          <button onClick={() => setShowInspector(true)} className="w-7 shrink-0 bg-slate-800 border-l border-slate-700 flex items-center justify-center text-slate-400 hover:text-white" title="Show Inspector">
      ...
      {activePanel && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/50" onClick={e => { if (e.target === e.currentTarget) setActivePanel(null); }}>
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700">
              <h2 data-testid="active-panel-title" className="font-semibold text-slate-100">
                ...
                {activePanel === 'photo-library' && (
                  <span data-testid="photo-library-title">Photo Library</span>
                )}
              </h2>
            ...
            <div className="p-5">
              {activePanel === 'color' && (
                <div>
                  ...
                  <div className="mt-4">
                    <label className="block text-xs text-slate-400 mb-1">Custom hex</label>
              ...
              {activePanel === 'photo' && (
                <div className="space-y-3 text-sm text-slate-300">
                  <p className="text-xs text-slate-400">Upload or select a site photo to attach to this board.</p>
                  <button className="w-full py-2 rounded bg-blue-600 hover:bg-blue-500 text-white text-xs">Upload photo</button>
                  <div className="grid grid-cols-3 gap-2 mt-2">
                    {[1,2,3,4,5].map(i => (
                      <div key={i} className="aspect-square rounded bg-slate-700 flex items-center justify-center cursor-pointer hover:bg-slate-600">
              ...
              {activePanel === 'scale' && (
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-xs text-slate-400 mb-1">Grid spacing (canvas units)</label>
              ...
              {activePanel === 'photo-library' && (
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[1,2,3,4,5].map(i => (
                      <div key={i} className="rounded bg-slate-700 aspect-video flex items-center justify-center cursor-pointer hover:bg-slate-600">
